...
BLACKLIST_REMOTE = 'https://pastebin.com/raw/U4nNdQsh'
CT_LOG = os.path.join(xbmc.translatePath('special://temp'), 'Protec-Kodi.log')
...
def updateBlacklist(cache, remote_source, fallback, force=False):
    ...
    if force or not os.path.isfile(cache) or (int(time.time()) - os.path.getmtime(cache) > limit):
        try:
            writeLog('Obtendo lista negra do provedor Gist')
            _src = urllib2.urlopen(remote_source, timeout=5).read()
            bl = 'online'
        except urllib2.URLError, e:
            writeLog('Nao foi possivel ler do provedor de essencia', xbmc.LOGFATAL)
            writeLog('%s' % (e.reason), xbmc.LOGFATAL)
            writeLog('Use a lista negra fornecida com a fonte adicional')
        ...
        writeLog('A lista negra esta atualizada')
    return bl