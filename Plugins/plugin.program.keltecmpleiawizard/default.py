...
INSTALLMETHODS   = ['Always Ask', 'Reload Profile', 'Forcar Fechar']
...
def buildMenu():
	...
	if bf == False:
		...
		addDir ('Guardar dados Menu'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def UNZIPROM():
	...
	if myroms == '':
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems that you do not have an extract location setup for Rom Packs" % COLOR2, "Voce gostaria de set one?[/COLOR]", yeslabel="[COLOR green][B]Set Location[/B][/COLOR]", nolabel="[COLOR red][B]Cancelar Download[/B][/COLOR]"):
			wiz.openS('rompath')
			myroms = wiz.getS('rompath')
			if myroms == '': return
	yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Tem certeza que voce gostaria de baixar e extrair [COLOR %s]%s[/COLOR] to:" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, myroms), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancelarl[/COLOR][/B]")
	if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: Install Cancelado[/COLOR]' % COLOR2); return
	display = name
			...
			if len(match) > 0:
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						selected = DIALOG.select("[COLOR %s]Addem umlready installed what Voce gostaria de do?[/COLOR]" % COLOR2, do)
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de remover the addon_data for:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR green]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]Nao pule[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						wiz.log("Repository not installed, Instalando it")
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Gostaria de instalar o repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR green]Yes Install[/COLOR][/B]", nolabel="[B][COLOR red]Nao pule[/COLOR][/B]"): 
							...
							if len(ver) > 0:
								...
								wiz.ebi('UpdateAddonRepos()')
								#wiz.ebi('UpdateLocalAddons()')
								wiz.log("Instalando Addon from Kodi")
						...
						wiz.log("No repository, Instalando addon")
						pluginid = plugin
						zipurl = url
						...
						wiz.log("Repository installed, Instalando addon")
...
def installAddon(name, url):
	...
	DP.update(0, title,'', '[COLOR %s]Por Favor Aguarde[/COLOR]' % COLOR2)
	percent, errors, error = extract.all(lib,ADDONS,DP, title=title)
	DP.update(0, title,'', '[COLOR %s]Instalando Dependencies[/COLOR]' % COLOR2)
	installed(name)
...
def misc():
		...
		addFile('Forcar Fechar Kodi',               'forceclose',      icon=ICONMAINT, themeit=THEME3)
...
def writeAdvanced(name, url):
	...
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de sobrescrever suas configuracoes avancadas atuais com [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Overwrite[/COLOR][/B]", nolabel="[B][COLOR red]Cancelarl[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de Baixar e Instalar [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Instalar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelarl[/COLOR][/B]")
		if choice == 1:
			file = wiz.openURL(url)
			f = open(ADVANCED, 'w'); 
			f.write(file)
			f.close()
			DIALOG.ok(ADDONTITLE, '[COLOR %s]AdvancedSettings.xml file has been Com sucesso written.  Once you click okay it will Forcar Fechar Kodi.[/COLOR]' % COLOR2)
			wiz.killxbmc(True)
		else: wiz.log("[Advanced Settings] install canceled"); wiz.LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), "[COLOR %s]Write Cancelado![/COLOR]" % COLOR2); return
...
def saveMenu():
	...
	addFile('Import Guardar dados',              'managedata', 'import', icon=ICONSAVE,  themeit=THEME1)
	addFile('Export Guardar dados',              'managedata', 'export', icon=ICONSAVE,  themeit=THEME1)
...
def FavsMenu():
	...
	if KEEPFAVS == 'true': addFile('Ultima gravacao: %s' % str(last), '', icon=ICONTRAKT, themeit=THEME3)
...
def traktMenu():
	...
	if KEEPTRAKT == 'true': addFile('Ultima gravacao: %s' % str(last), '', icon=ICONTRAKT, themeit=THEME3)
...
def allucMenu():
	...
	if KEEPALLUC == 'true': addFile('Ultima gravacao: %s' % str(last), '', icon=ICONLOGIN, themeit=THEME3)
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de [COLOR %s]Forcar Fechar[/COLOR] kodi or [COLOR %s]Reload Profile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR red]Forcar Fechar[/COLOR][/B]")
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]AVISO!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Voce gostaria de apply the guiFix anyways?.[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		if yes:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]A correcao Gui foi instalada.  Gostaria de recarregar o perfil ou Forcar Fechar Kodi?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR green]Forcar Fechar[/COLOR][/B]")
			if todo == 1: wiz.reloadFix()
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Para salvar as alteracoes, voce precisa agora Forcar Fechar Kodi, Pressione OK para Forcar Fechar Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if warning == True:
			yes_pressed = DIALOG.yesno("%s - [COLOR red]AVISO!![/COLOR]" % ADDONTITLE, '[COLOR %s]Ha uma chance de que a pele nao apareca corretamente' % COLOR2, 'Quando Instalando um %s build em um Kodi %s install' % (wiz.checkBuild(name, 'kodi'), KODIV), 'Voce ainda gostaria de instalar: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Sim, Instalar[/COLOR][/B]')
		else:
			if not over == False: yes_pressed = 1
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de Baixar e Instalar:' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Sim, Instalar[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			#buildzips = []
			...
			if int(float(percent)) > 0:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Para salvar as alteracoes, voce precisa agora Forcar Fechar Kodi, Pressione OK para Forcar Fechar Kodi[/COLOR]" % COLOR2); wiz.killxbmc('true')
			else:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox("%s: Error Instalando Build" % ADDONTITLE, error)
		else:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Build Install: Cancelado![/COLOR]' % COLOR2)
	elif type == 'theme':
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Build [COLOR %s]%s[/COLOR] comes with [COLOR %s]%s[/COLOR] different themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Voce gostaria de install one now?[/COLOR]", yeslabel="[B][COLOR green]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]"):
						...
						else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Install: Cancelado![/COLOR]' % COLOR2); return
					else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Install: Cancelado![/COLOR]' % COLOR2); return
			else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Install: None Found![/COLOR]' % COLOR2)
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Gostaria de instalar o theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR green]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]")
		if installtheme:
			...
			if url not in ["fresh", "normal"]:
				...
				if test == True:
					...
					xbmc.sleep(500)
			title = '[COLOR %s][B]Instalando Theme:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, theme)
			DP.update(0, title,'', 'Por Favor Aguarde')
			percent, errors, error = extract.all(lib,HOME,DP, title=title)
			...
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Install: Cancelado![/COLOR]' % COLOR2)
def thirdPartyInstall(name, url):
	...
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de preform a [COLOR %s]Fresh Install[/COLOR] or [COLOR %s]Normal Install[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR green]Fresh Install[/COLOR][/B]", nolabel="[B][COLOR red]Normal Install[/COLOR][/B]")
	if type == 1:
	...
	if int(float(percent)) > 0:
		...
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Voce gostaria de view the errors?[/COLOR]', nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR green]View Errors[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android'  or DEVELOPER == 'true':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de Baixar e Instalar:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancelarl[/COLOR][/B]")
		if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: Install Cancelado[/COLOR]' % COLOR2); return
		display = apk
		if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
...
def apkInstaller1(apk, url):
	if wiz.platform() == 'android' or DEVELOPER == 'true':
		...
		if yes:
			...
			DP.create(ADDONTITLE,'Baixando %s' % display,'', 'Por Favor Aguarde')
			lib=os.path.join(PACKAGES, "%s.apk" % apk.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', ''))
			try: os.remove(lib)
			except: pass
			downloader.download(url, lib, DP)
		...
		else: wiz.LogNotify(ADDONTITLE, '[COLOR red]ERROR:[/COLOR] Install Cancelado')
...
def viewLogFile():
	...
	if not oldlog == False and not mainlog == False:
		...
		if which == -1: wiz.LogNotify('[COLOR %s]View Log[/COLOR]' % COLOR1, '[COLOR %s]View Log Cancelado![/COLOR]' % COLOR2); return
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce tem certeza que quer to delete the addon:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Remove Addon[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]')
	if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de remover [COLOR %s]ALL[/COLOR] addon data stored in you Userdata folder?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
			wiz.cleanHouse(ADDOND)
		else: wiz.LogNotify('[COLOR %s]Remove Addon Data[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
	elif addon == 'uninstalled':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de remover [COLOR %s]ALL[/COLOR] addon data stored in you Userdata folder for uninstalled addons?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
			total = 0
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de remover [COLOR %s]ALL[/COLOR] pastas de dados addon vazias na sua pasta Userdata?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
		...
		else: wiz.LogNotify('[COLOR %s]Remove Empty Folders[/COLOR]' % COLOR1, '[COLOR %s]Cancelado![/COLOR]' % COLOR2)
	else:
		...
		if addon in EXCLUDES:
			...
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce tambem gostaria de remover the addon data for:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
...
def restoreextit(type):
	if type == 'build':
		x = freshStart('restore')
		if x == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]External Restore Cancelado[/COLOR]" % COLOR2); return
...
def addonpackwiz():
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Gostaria de instalar o:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B]Nao, Cancelar[/B]',yeslabel='[B]Instalar[/B]')
		if yes:
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Baixando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Por Favor Aguarde')
			...
			xbmc.sleep(500)
			DP.update(0,"", "Instalando %s " % name)
			title = '[COLOR %s][B]Instalando Addon Pack:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
			DP.update(0, title,'', 'Por Favor Aguarde')
			percent, errors, error = extract.all(lib,HOME,DP, title=title)
			wiz.log('INSTALLED %s: [ERRORS:%s]' % (percent, errors))
			DP.close()
			#DIALOG.ok(ADDONTITLE, "[COLOR %s]Addon Pack installed, Voce precisa reset Kodi, Pressione OK para reset Kodi[/COLOR]" % COLOR2)
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Instalado:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name),'[COLOR green]Com sucesso[/COLOR]', nolabel='[B]Restabelecer[/B]',yeslabel='[B]Forcar Fechar[/B]')
			if yes == 1:
				wiz.killxbmc('true')
			elif yes == 0:
				wiz.RESET()
		else:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Addon Pack: Cancelado![/COLOR]' % COLOR2)
...
def manageSaveData(do):
	if do == 'import':
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce gostaria de substituir o atual [COLOR %s]%s[/COLOR] Arquivo?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR green]Sim substituir[/COLOR][/B]", nolabel="[B][COLOR red]Nao pule[/COLOR][/B]"): continue
					else: os.remove(old)
		...
		os.remove(tempfile)
		if x == 0: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Guardar dados Importacao falhou[/COLOR]" % COLOR2)
		else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Guardar dados Importacao Completa[/COLOR]" % COLOR2)
	elif do == 'export':
		...
		zipf.close()
		if source == mybuilds:
			DIALOG.ok(ADDONTITLE, "[COLOR %s]Salvar dados foi feito backup para:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
		else:
			try:
				xbmcvfs.copy(tempzip, os.path.join(source, 'SaveData.zip'))
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Salvar dados foi feito backup para:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, os.path.join(source, 'SaveData.zip')))
			except:
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Salvar dados foi feito backup para:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
###########################
###### Fresh Install ######
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Click [B][COLOR springgreen] - Sim - [/COLOR][/B]" % COLOR2, "Para apagar sua Build atual, \r\nEm seguida, instale um backup de Build armazenado local ou externo[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Sim[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Click [B][COLOR springgreen] - Sim - [/COLOR][/B]" % COLOR2, "Para apagar sua Build atual, \r\nEntao Fresh Install [COLOR %s]%s[/COLOR]!!" % (COLOR1, install), nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Sim[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce deseja restaurar seu" % COLOR2, "Configuracao para as configuracoes padrao?[/COLOR]", nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Sim[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
		...
		DP.update(0, "[COLOR %s]Expulsando arquivos e pastas:" % COLOR2)
		...
		if over == True:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce precisa [COLOR %s]Forcar Fechar[/COLOR] Este aplicativo [COLOR %s]And[/COLOR] Entao reinicie-o novamente[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR white]>>>>>>[/COLOR][/B]", nolabel="[B][COLOR springgreen]Forcar Fechar[/COLOR][/B]")
...
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de Excluir cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Excluir Cache[/COLOR][/B]'):
		wiz.clearCache()
		DC.setLabel('Size: [B][COLOR lime]0.0 B[/B][/COLOR]')
def clearArchive():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de Excluir the \'Archive_Cache\' folder?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Sim excluir[/COLOR][/B]'):
		wiz.clearArchive()
def clearPackages():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de Excluir pacotes?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Nao, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Delete[/COLOR][/B]'):
		wiz.clearPackages('total')
		DPK.setLabel('Size: [B][COLOR lime]0.0 B[/B][/COLOR]')
		TPK.setLabel('Files: [B][COLOR lime]0[/B][/COLOR]')

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de Excluir cache, packages and thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancel Process[/COLOR][/B]',yeslabel='[B][COLOR green]Clean All[/COLOR][/B]'):
		wiz.clearCache()
		wiz.clearPackages('total')
		clearThumb('total')
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Voce gostaria de delete the %s and Thumbnails folder?' % (COLOR2, latest), "They will repopulate on the next startup[/COLOR]", nolabel='[B][COLOR red]Don\'t Delete[/COLOR][/B]', yeslabel='[B][COLOR green]Delete Thumbs[/COLOR][/B]')
	if choice == 1:
		...
		DTH.setLabel('Size: [B][COLOR lime]0.0 B[/B][/COLOR]')
		TTH.setLabel('Files: [B][COLOR lime]0[/B][/COLOR]')
	else: wiz.log('Clear thumbnames Cancelado')
	wiz.redoThumbs()
def purgeDb():
	DB = []; display = []
	...
	if KODIV >= 16: 
		...
		if choice == None: wiz.LogNotify("[COLOR %s]Purge Database[/COLOR]" % COLOR1, "[COLOR %s]Cancelado[/COLOR]" % COLOR2)
		elif len(choice) == 0: wiz.LogNotify("[COLOR %s]Purge Database[/COLOR]" % COLOR1, "[COLOR %s]Cancelado[/COLOR]" % COLOR2)
		else: 
			for purge in choice: wiz.purgeDb(DB[purge])
	else:
		choice = DIALOG.select("[COLOR %s]Select DB File to Purge[/COLOR]" % COLOR2, display)
		if choice == -1: wiz.LogNotify("[COLOR %s]Purge Database[/COLOR]" % COLOR1, "[COLOR %s]Cancelado[/COLOR]" % COLOR2)
...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		PreviewButton = pyxbmct.Button('[COLOR %s][B]Video OFF[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(PreviewButton,20 , 20, 8, 8)
		window.connect(PreviewButton,lambda: buildVideo(Bname))
		
		InstallButton = pyxbmct.Button('[COLOR %s][B]Instalar[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(InstallButton,28 , 20, 8, 8)
		window.connect(InstallButton, lambda: buildWizard(Bname,'normal'))
		
		FreshStartButton = pyxbmct.Button('[COLOR %s][B]Fresh Instalar[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(FreshStartButton,36 , 20, 8, 8)
		window.connect(FreshStartButton,lambda: buildWizard(Bname,'fresh'))
		
		ThemeButton = pyxbmct.Button('[COLOR %s][B]Instalar Temas[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		...
		buildname  = pyxbmct.Label('[COLOR %s]Nome Atual Da Build: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildname))
		window.placeControl(buildname, 90, 2, 8, 25)
		
		buildversion  = pyxbmct.Label('[COLOR %s]Versao atual da Build: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildversion))
		...
		buildtheme  = pyxbmct.Label('[COLOR %s]Build Tema: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildtheme))
		window.placeControl(buildtheme, 90, 28, 8, 21)
		
		errorinstall  = pyxbmct.Label('[COLOR %s]Erros Durante Instalacao: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,errors))
		window.placeControl(errorinstall, 95, 28, 8, 20)
		
		lastupdatchk  = pyxbmct.Label('[COLOR %s]Ultima verificacao de atualizacao: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,lastbuildcheck))
...
def AddonList():
	...
	if not ADDONFILE == 'http://' and not ADDONFILE == '':
		...
		#buttons/objects
		InstallButtonA = pyxbmct.Button('[COLOR %s][B]Instalar[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def APKList():
	...
	if not APKFILE == 'http://' and not APKFILE == '':
		...
		InstallButtonAPK = pyxbmct.Button('[COLOR %s][B]Instalar[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def EmuList():
	...
	if not EMUAPKS == 'http://' and not EMUAPKS == '':
		...
		InstallButtonEMU = pyxbmct.Button('[COLOR %s][B]Instalar[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def RomList():
	...
	if not ROMPACK == 'http://' and not ROMPACK == '':
		...
		InstallButtonROM = pyxbmct.Button('[COLOR %s][B]Instalar[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def Maint():
	...
	#buttons#
	total_clean_button = pyxbmct.Button('[COLOR %s]Limpeza total[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_clean_button, 72 , 21 ,  9, 8)
	window.connect(total_clean_button,lambda: totalClean())
	### I'm good, u?
	total_cache_button = pyxbmct.Button('[COLOR %s]Excluir cache[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_cache_button, 87 , 21 ,  9, 9)
	window.connect(total_cache_button,lambda: clearCache())
	
	total_packages_button = pyxbmct.Button('[COLOR %s]Excluir pacotes[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_packages_button, 87 , 31 ,  9, 9)
	window.connect(total_packages_button,lambda: clearPackages())
	
	total_thumbnails_button = pyxbmct.Button('[COLOR %s]Excluir miniaturas[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	settings_button1 = pyxbmct.Button('[COLOR %s]Definicoes[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def BackRes():
	...
	favs = pyxbmct.Label('[B][COLOR %s]Ultima gravacao:[/COLOR] [COLOR %s]%s[/COLOR][/B]' % (DES_T_COLOR, DESCOLOR,str(last)))
	...
	#backup#
	Backup = pyxbmct.Label('[B][COLOR %s]Ferramentas de Backup:[/COLOR][/B]' % DES_T_COLOR)
	...
	savefav_button = pyxbmct.Button('[COLOR %s] Favoritos[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(savefav_button, 82, 3, 10, 10)
	window.connect(savefav_button,lambda: wiz.BACKUPFAV())
	
	#restore#
	restore_title = pyxbmct.Label('[B][COLOR %s]Restaurar Ferramentas:[/COLOR][/B]' % DES_T_COLOR)
	...
	restorefav_button = pyxbmct.Button('[COLOR %s]Favoritos[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(restorefav_button, 72, 20, 10, 10)
	window.connect(restorefav_button,lambda: wiz.RESTOREFAV())
	
	#clear backups#
	delete_title = pyxbmct.Label('[B][COLOR red]Excluir ferramentas:[/COLOR][/B]')
	window.placeControl(delete_title, 54, 37, 10, 10)
	
	clearfav_button = pyxbmct.Button('[COLOR %s]Excluir Favoritos[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(clearfav_button, 64, 37, 10, 10)
	window.connect(clearfav_button,lambda: wiz.DELFAV())
	
	clear_backup_button = pyxbmct.Button('[COLOR %s]Excluir Back-ups[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(clear_backup_button, 74, 37, 10, 10)
	window.connect(clear_backup_button,lambda: wiz.cleanupBackup())
	
	#settings
	set_title = pyxbmct.Label('[B][COLOR %s]Definicoes:[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(set_title, 32, 37, 10, 10)
	
	settings_button = pyxbmct.Button('[COLOR %s]Definicoes[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(settings_button, 42, 37, 10, 10)
	window.connect(settings_button, lambda: wiz.openS('Manutencao'))
...
def Tools():
	...
	Advan = pyxbmct.Label('[B][COLOR %s]Ferramentas avancadas de configuracoes[/COLOR][/B]'% DES_T_COLOR)
	window.placeControl(Advan, 15, 20, 9, 13)
	#buttons#
	autoadvanced_buttonQ = pyxbmct.Button('[COLOR %s]Configuracao rapida[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(autoadvanced_buttonQ, 22, 21, 9, 9)
	window.connect(autoadvanced_buttonQ,lambda: notify.autoConfig2())
	
	autoadvanced_button = pyxbmct.Button('[COLOR %s]Configuracao total[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(autoadvanced_button, 31, 21, 9, 9)
	window.connect(autoadvanced_button,lambda: notify.autoConfig())
	##Wait it's here!!
	currentsettings_button = pyxbmct.Button('[COLOR %s]Configuracoes atuais[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(currentsettings_button, 40, 21, 9, 9)
	window.connect(currentsettings_button,lambda: viewAdvanced())
	
	removeadvanced_button = pyxbmct.Button('[COLOR %s]Excluir configuracoes[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def Installables():
	...
	EmuButton = pyxbmct.Button('[COLOR %s][B]Emuladores[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
...
MaintButton = pyxbmct.Button('[COLOR %s][B]Manutencao[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(MaintButton,2 , 9 , 9, 8)
window.connect(MaintButton, lambda: Maint())

BackResButton = pyxbmct.Button('[COLOR %s][B]Backup/Restaurar[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(BackResButton,2 , 17, 9, 8)
window.connect(BackResButton, lambda: BackRes())

ToolsButton = pyxbmct.Button('[COLOR %s][B]Ferramentas[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(ToolsButton,2 , 25, 9, 8)
window.connect(ToolsButton, lambda: Tools())

InstallablesButton = pyxbmct.Button('[COLOR %s][B]Instalar APKs[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(InstallablesButton,2 , 33, 9, 8)
window.connect(InstallablesButton, lambda: Installables())

CloseButton = pyxbmct.Button('[COLOR %s][B]Fechar[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
